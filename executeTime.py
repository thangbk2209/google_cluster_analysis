...
for file_name in os.listdir(folder_path):
    ...
    if(file_name_data[0] == 175):
        if(timePoint > minMaxTimeArr[1][0] && timePoint < minMaxTimeArr[0][1]):
            part_name = "/mnt/volume/ggcluster/spark-2.1.1-bin-hadoop2.7/thangbk2209/TopJobId/JobMaxTaskpart-00"+str(file_name_data[0]+1).zfill(3)+"-of-00500.csv"
            df = (
                sql_context.read
            ...
            newData.toPandas().to_csv('thangbk2209/tenSecondsTopJobId/%s-%s.csv'%(file_name_data[0]+1,timePoint), index=False, header=None)
    elif(file_name_data[0] == 270):
        if(timePoint > minMaxTimeArr[file_name_data[0]-175][0] && timePoint < minMaxTimeArr[file_name_data[0]-176][1]):
            part_name = "/mnt/volume/ggcluster/spark-2.1.1-bin-hadoop2.7/thangbk2209/TopJobId/JobMaxTaskpart-00"+str(file_name_data[0]-1).zfill(3)+"-of-00500.csv"
            df = (
                sql_context.read
                .format('com.databricks.spark.csv')
        ...
        if(timePoint < minMaxTimeArr[file_name_data[0]-175][1] && timePoint > minMaxTimeArr[file_name_data[0]-174][0]):
            part_name = "/mnt/volume/ggcluster/spark-2.1.1-bin-hadoop2.7/thangbk2209/TopJobId/JobMaxTaskpart-00"+str(file_name_data[0]+1).zfill(3)+"-of-00500.csv"
            df = (
                sql_context.read
                .format('com.databricks.spark.csv')
